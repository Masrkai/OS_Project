...
int main(int argc, char *argv[]) {
  ...
  if (schedulerPid == 0) {
    ...
    execl("./scheduler.out", "scheduler.out", algoStr, quantumStr, msgqStr,
          NULL);
...
int readProcesses(const char *filename, Process processes[]) {
  ...
  while (fgets(line, sizeof(line), file) != NULL && count < MAX_PROCESSES) {
    ...
    // Parse process data
    int result = sscanf(line, "%d\t%d\t%d\t%d", &processes[count].id,
    ...
    if (result == 4) {
      printf("Process %d: arrival=%d, runtime=%d, priority=%d\n",
             processes[count].id, processes[count].arrivalTime,
             processes[count].runtime, processes[count].priority);
      count++;
    }
  }
